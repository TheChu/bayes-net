...
def joinFactors(factors):
    ...
    setsOfConditioned = [set(factor.conditionedVariables()) for factor \
                         in factors]
    setOfConditioned = set().union(*setsOfConditioned) - setOfUnconditioned
    variableDomainsDict = factors[0].variableDomainsDict()
    joinedFactor = Factor(setOfUnconditioned, setOfConditioned,
                          variableDomainsDict)

    # Get product of probabilities for every assignment
    for assignment in joinedFactor.getAllPossibleAssignmentDicts():
        probProduct = reduce(lambda x, y: x * y,
                             [factor.getProbability(assignment) for factor \
                              in factors])
        joinedFactor.setProbability(assignment, probProduct)
